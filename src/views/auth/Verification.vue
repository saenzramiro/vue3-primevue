...
<template>
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 800" class="fixed left-0 top-0 min-h-screen min-w-screen" preserveAspectRatio="none">
		...
		/>
		<path
			:fill="darkMode ? 'var(--primary-500)' : 'var(--primary-100)'"
		...
		<div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
			...
			<div class="flex flex-column">
				<div class="flex justify-content-between w-full align-items-center mb-4 gap-3">
					<InputNumber v-model="value1" input-id="val1" input-class="w-3rem text-center" :max="9" @keyup="focus($event)"></InputNumber>
					<InputNumber v-model="value2" input-id="val2" input-class="w-3rem text-center" :max="9" @keyup="focus($event)"></InputNumber>
					<InputNumber v-model="value3" input-id="val3" input-class="w-3rem text-center" :max="9" @keyup="focus($event)"></InputNumber>
					<InputNumber v-model="value4" input-id="val4" input-class="w-3rem text-center" :max="9"></InputNumber>
				</div>
